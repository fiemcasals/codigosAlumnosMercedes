# Juego de buscaminas en tkinter, donde el usuario puede hacer clic en las casillas para descubrirlas. con el boton derecho se marca la casilla que coniene una mina y con el izquierdo se descubre la casilla. usando emojis en lugar de imágenes.
import random
...
class Buscaminas:
        ...
        for i in range(self.filas):
            for j in range(self.columnas):
                boton = tk.Button(
                    self.root,
                    text='',
                    width=4,   # Ajusta este valor para que el botón sea cuadrado visualmente
                    height=2,  # Mantén el alto actual
                    font=("Arial", 8),  # Tamaño de fuente para los emojis
                    command=lambda x=i, y=j: self.descubrir_casilla(x, y)
                )
    ...
    def marcar_casilla(self, event, fila, columna):
        ...
        if boton['text'] == '' and not boton['image']:
            boton.config(text=self.emoji_bandera)
            boton['state'] = 'disabled'
        elif boton['text'] == self.emoji_bandera:
            boton.config(text='')
            boton['state'] = 'normal'
        ...
        for i in range(self.filas):
            for j in range(self.columnas):
                ...
                if self.tablero[i][j] == -1:
                    boton.config(text=self.emoji_mina)
                    boton['state'] = 'disabled'
                else:
                    boton['state'] = 'disabled'
    ...
    def descubrir_casilla(self, fila, columna):
        ...
        if self.tablero[fila][columna] == -1:
            self.botones[fila][columna].config(text=self.emoji_explosion)
    ...
    def revelar_casilla(self, fila, columna):
        ...
        if self.tablero[fila][columna] == 0:
            self.botones[fila][columna].config(text=self.emoji_casilla_vacia)
        else:
            self.botones[fila][columna].config(text=str(self.tablero[fila][columna]))
